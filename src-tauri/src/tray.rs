...
pub enum AppTheme {
    Dark,
    Light,
    Colored, // Blue/colored theme for Linux
}
...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState, warning: bool) -> &'static str {
    ...
    match (theme, state) {
        ...
        // Colored theme uses blue icons (for Linux)
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Handy Codex v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Handy Codex v{}", env!("CARGO_PKG_VERSION"))
    }
}
