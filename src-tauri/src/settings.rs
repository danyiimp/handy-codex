...
mod tests {
    ...
    #[test]
    fn legacy_codex_settings_keep_auth_and_toggle_behavior_after_migration() {
        let stored = serde_json::json!({
            "settings_schema_version": 1,
            "selected_model": "codex-chatgpt-asr",
            "codex_auth_file": "/custom/codex/auth.json",
            "push_to_talk": false,
            "audio_feedback": false,
            "mute_while_recording": true,
            "post_process_enabled": true,
            "post_process_provider_id": "codex_chatgpt",
            "post_process_models": { "codex_chatgpt": "custom-model" },
        ...
        let mut settings: AppSettings = serde_json::from_value(stored.clone())
            .expect("the pre-sync Codex settings schema must still parse");

        assert!(apply_settings_migrations(&mut settings, &stored));
        ensure_post_process_defaults(&mut settings);

        assert_eq!(settings.selected_model, "codex-chatgpt-asr");
        assert_eq!(
            settings.codex_auth_file.as_deref(),
            Some("/custom/codex/auth.json")
        );
        ...
        assert_eq!(settings.post_process_provider_id, "codex_chatgpt");
        assert_eq!(
            settings.post_process_models["codex_chatgpt"],
            "custom-model"
        );
        assert_eq!(
            settings.active_post_process_provider().unwrap().base_url,
        ...
        let mut reloaded: AppSettings = serde_json::from_value(migrated.clone()).unwrap();
        assert!(!apply_settings_migrations(&mut reloaded, &migrated));
        assert!(!ensure_post_process_defaults(&mut reloaded));
        assert_eq!(serde_json::to_value(reloaded).unwrap(), migrated);
    }
