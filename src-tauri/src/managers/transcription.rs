        ...
        let (result, output_language, model_languages) = {
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                match &mut engine {
                    ...
                    LoadedEngine::CodexAsr(client) => {
                        // The remote request includes every explicit language
                        // hint, even without a published language capability list.
                        applied_language_hint = Some(validated_language.clone());
                        client
...
mod tests {
    ...
    #[test]
    fn remote_language_hint_controls_fillers_without_model_language_metadata() {
        let settings = AppSettings {
            selected_language: "en".to_string(),
            ..Default::default()
        };
        let evidence = resolve_output_language_evidence(&settings, Some("en"), &[], false);
        let result =
            post_process_transcription_text("um ok".to_string(), &settings, false, &evidence, &[]);
        ...
        assert_eq!(result, "ok");
        assert_eq!(
            resolve_output_language_evidence(&settings, Some("auto"), &[], false),
            OutputLanguageEvidence::Unknown
        );
    }
